  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-lg border border-brown/10 flex flex-col hover:shadow-xl transition-shadow group">
      ...
      <div className="p-6 flex-1 flex flex-col">
        <h3 className="text-2xl font-serif font-bold text-brown mb-2">{product.name}</h3>
        <p className="text-brown/70 mb-4 flex-1 line-clamp-3">{product.description}</p>
        
        {highlights.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {highlights.map((h, i) => (
               // Prevent empty strings from rendering padding
               h.trim() && (
                  <span key={i} className="text-xs bg-cream text-brown px-3 py-1 rounded-full font-medium border border-brown/10 tracking-wide">
                    {h.trim()}
                  </span>
               )
            ))}
